...
unzip("./data/household_power_consumption.zip", files = NULL, list = FALSE, overwrite = TRUE, junkpaths = FALSE, exdir = "./data", unzip = "internal", setTimes = FALSE)

###Extract to a dataframe called df only those observations from the dataset that refer to 1 and 2 February 2007 
###(can be skipped if you've already run plot1.R)
...
###Use paste() to paste Date and Time variable to one new variable. Use strptime() and as.Date() to convert the new 
###character variable describing Date-Time to a date-time format (can be skipped if you've already run Plot1.R)
df$Datetime<-paste(df$Date, df$Time)
df$Datetime2<-as.POSIXct(strptime(df$Datetime, format="%e/%m/%Y %H:%M:%S"))

###Open Reshape2-library in order to be able to use melt() to put the three Sub_metering variables into one new variable. 
###This is necessary, or at least is my solution, for getting all three Sub_metering variables into one graph 
###(can be skipped if you've already run Plot3.R)
library(reshape2)
...
png(file="./data/plot4.png")
  ...
  legend("topright", box.lwd=0,lty=1, col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_meter  ing_3"))
